...
ONTAP 더 이상 전문 서비스 업체가 주도하는 마이그레이션을 요구하지 않지만, NetApp 가장 간단한 마이그레이션을 제외한 모든 마이그레이션에 대해 범위 설정, 계획 및 교육에 전문 서비스 업체가 참여할 것을 강력히 권장합니다.

FLI는 SAN LUN을 ONTAP로 마이그레이션하도록 개발되었습니다. FLI는 다음을 포함하되 이에 국한되지 않는 다양한 마이그레이션 요구사항을 지원합니다.

* EMC, Hitachi, HP 및 기타 공급업체의 이기종 스토리지 어레이에서 NetApp 으로 데이터를 마이그레이션합니다.
