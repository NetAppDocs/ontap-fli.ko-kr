...
summary: FLI는 FC LUN만 지원합니다. 하지만 iSCSI LUN을 가져올 수 있는 해결 방법이 있습니다. 다른 FLI 온라인 7-모드 ONTAP 워크플로와 달리, iSCSI LUN을 FC LUN으로 가져오기 때문에 중단 기간은 전체 워크플로에 걸쳐 발생합니다. 
---
= 비 FC LUN 임포트
:allow-uri-read: 
:icons: font
...
[role="lead"]
FLI는 FC LUN만 지원합니다. 하지만 iSCSI LUN을 가져올 수 있는 해결 방법이 있습니다. 다른 FLI 온라인 7-모드 ONTAP 워크플로와 달리, iSCSI LUN을 FC LUN으로 가져오기 때문에 중단 기간은 전체 워크플로에 걸쳐 발생합니다.
